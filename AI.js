...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const startCoolDown = (seconds) => {
        ...
        // Save original children to restore later — no innerHTML needed
        const originalChildren = Array.from(sendBtn.childNodes).map(n => n.cloneNode(true));

        // Build countdown span safely using DOM
        const countSpan = document.createElement('span');
        countSpan.style.cssText = 'font-size:.82rem;font-weight:700;color:#fff';

        let timeLeft = seconds;
        countSpan.textContent = `${timeLeft}s`;
        sendBtn.replaceChildren(countSpan);
        ...
        const t = setInterval(() => {
            if (--timeLeft < 0) {
                ...
                if (sendBtn) countSpan.textContent = `${timeLeft}s`;
            }
        }, 1000);
    };
    ...
    const generateResponse = async (botDiv) => {
        ...
        // If there is a file, include it as text description
        // (file analysis is handled by whichever AI provider supports it)
    ...
    document.getElementById("delete-chats-btn")?.addEventListener("click", () => {
        ...
        chatHistory = [];
        chatsContainer.replaceChildren(); // safer than innerHTML = ""
